...
vehicle_fleet_flag_copert=['Small','Medium','Large','Small','Medium','Large','Motorcycles 4-stroke 250 - 750 cm³','Urban Buses Standard 15 - 18 t','Small']


vehicle_fleet_flag=['LDV','MDV','HDV','LDT','MDT','HDT','MC','Bus','Taxi']
vehicle_fleet=np.loadtxt(work_path + '/base_info/vehicle_fleet.csv',delimiter = ',', skiprows = 1,usecols=(1,))

fuel_type_flag=['Petrol','Diesel','LPG']
fuel_type_percentage=np.loadtxt(work_path + '/base_info/fuel_type_percentage.csv',delimiter = ',', skiprows = 1,usecols=(1,2,3))
fuel_type_percentage=fuel_type_percentage/100

emission_standard_flag=['Pre I','CHINA I','CHINA II','CHINA III','CHINA IV','CHINA V']


emission_standard_percentage=np.loadtxt(work_path + '/base_info/emission_standard_percentage.csv',delimiter = ',', skiprows = 1,usecols=(1,2,3,4,5,6))
...
f_emission_factor=open(work_path + '/base_info/simplified-new-Road Transport Hot Emission Factor chinese.csv', 'r', encoding='utf-8')
...
for n in range(span_day):
    ...
    speed_data = np.loadtxt(work_path + '/speed_hour/' \
                        ...
                        for es in range(len(emission_standard_flag)):
                            ...
                            for ef in range(len(emission_factor)):
                                ...
                                if emission_factor[ef][0] == 'Passenger Cars' and emission_factor[ef][1] == fuel_type_flag[ft] and \
                                        emission_factor[ef][2] == vehicle_fleet_flag_copert[vf] and emission_factor[ef][3] == emission_standard_flag[es] and \
                        ...
                        for es in range(len(emission_standard_flag_truck)):
                            ...
                            for ef in range(len(emission_factor)):
                                ...
                                if emission_factor[ef][0] == vehicle_fleet_flag_truck and emission_factor[ef][1] == fuel_type_flag[ft] and \
                                        emission_factor[ef][2] == vehicle_fleet_flag_copert[vf] and emission_factor[ef][3] == emission_standard_flag_truck[es] and \
                                        emission_factor[ef][5] == species_flag[s]:
    ...
    for s_out in range(len(species_flag)):
        save_output1 = np.savetxt(work_path + 'result/emission_hour/' \
    ...
    for fuel_type_output in range(len(fuel_type_flag)):
        scio.savemat(work_path + 'result/emission_hour/' + \
                     str(cal_day.year).zfill(4) + '-' + str(cal_day.month).zfill(2) + '-' + str(
