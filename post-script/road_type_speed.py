...
work_path = './ROE_V1.0/'
data_path= work_path + '/calculation/speed_hour/'
output_path = work_path + '/calculation/speed_hour/'
...
for n in range(span_day):
    ...
    for t in range(len(road_level)):
        road_type_speed_ave[t, :] = np.nanmean(road_type_speed[:, :, t], 0)

    print(road_type_speed_ave)

    save_output1 = np.savetxt(output_path \
                              + str(cal_day.year).zfill(4) + '-' + str(cal_day.month).zfill(2) + '-' \
                              + str(cal_day.day).zfill(2) + '_road_type_speed_ave.txt', \
                              road_type_speed_ave, fmt='%2f', delimiter=',', \
